...
)  # we provide the path for the Command.check method
class CommandTests(SimpleTestCase):
    ...
    def test_wait_for_db_ready(self, patched_check):
        ...
        patched_check.return_value = (
            True  # this means when check is called, we return the true value
        )
        ...
        patched_check.assert_called_once_with(
            databases=["default"]
        )  # the mock value of check is called by default
